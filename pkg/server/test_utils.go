...
func prepareTests(t *testing.T) (*client.ColoniesClient, *ColoniesServer, string, chan bool) {
	...
	server := CreateColoniesServer(db, TESTPORT, serverID, EnableTLS, "../../cert/key.pem", "../../cert/cert.pem", node, clusterConfig, "/tmp/colonies/etcd", GENERATOR_TRIGGER_PERIOD, CRON_TRIGGER_PERIOD)
...
func StartCluster(t *testing.T, db database.Database, size int) []ServerInfo {
	...
	for i, node := range clusterConfig.Nodes {
		go func(i int, node cluster.Node) {
			...
			server := CreateColoniesServer(db, node.APIPort, serverID, false, "", "", node, clusterConfig, "/tmp/colonies/etcd"+strconv.Itoa(i), GENERATOR_TRIGGER_PERIOD, CRON_TRIGGER_PERIOD)
