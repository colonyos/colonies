...
// kernel-assigned ports so tests can run in parallel.
func testServerManagerNode(t *testing.T) cluster.Node {
	ports, err := utils.FreePorts(4)
	assert.Nil(t, err)
	return cluster.Node{
		Name:           "test-node",
		Host:           "localhost",
		EtcdClientPort: ports[0],
		EtcdPeerPort:   ports[1],
		RelayPort:      ports[2],
		APIPort:        ports[3],
	}
}
