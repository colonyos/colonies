...
func TestHandlerRegistry(t *testing.T) {
	...
	// Test registration
	testHandler := func(c backends.Context, recoveredID string, payloadType string, jsonString string) {
...
func TestHandlerRegistryRequest(t *testing.T) {
	...
	ginCtx := ginbackends.NewContext(ctx)

	handled := registry.HandleRequest(ginCtx, "test_id", "test_payload", "{}")
	...
	ginCtx2 := ginbackends.NewContext(ctx2)

	handled = registry.HandleRequest(ginCtx2, "test_id", "unknown_payload", "{}")
