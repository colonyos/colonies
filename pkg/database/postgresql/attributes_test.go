...
func TestDeleteAttributesByColonyIDWithState(t *testing.T) {
	...
	attribute6 := core.CreateAttribute(process6.ID, colonyID, "", core.IN, "test_key1", "test_value1")
	err = db.AddAttribute(attribute6)
	assert.Nil(t, err)
	...
	attributeFromDB, err = db.GetAttributeByID(attribute3.ID)
	assert.Nil(t, err)
	assert.Equal(t, attributeFromDB, attribute3)
	...
	attributesFromDB, err := db.GetAttributesByColonyID(colonyID)
	assert.Nil(t, err)
	assert.Len(t, attributesFromDB, 2) // 1 running process and 1 process with process graph == 2 processes
