...
func TestExecutorWithNodeID(t *testing.T) {
	db, err := PrepareTests()
	assert.Nil(t, err)
	defer db.Close()
	...
	// Create executor with NodeID
	executor := utils.CreateTestExecutor(colony.Name)
	executor.NodeID = node.ID
	err = db.AddExecutor(executor)
	assert.Nil(t, err)

	// Verify executor has NodeID
	executorFromDB, err := db.GetExecutorByID(executor.ID)
	assert.Nil(t, err)
	assert.NotNil(t, executorFromDB)
	assert.Equal(t, node.ID, executorFromDB.NodeID)
}

func TestExecutorWithoutNodeID(t *testing.T) {
	db, err := PrepareTests()
	assert.Nil(t, err)
	defer db.Close()
	...
	// Create executor without NodeID (backward compatibility)
	executor := utils.CreateTestExecutor(colony.Name)
	executor.NodeID = ""
	err = db.AddExecutor(executor)
	assert.Nil(t, err)

	// Verify executor has empty NodeID
	executorFromDB, err := db.GetExecutorByID(executor.ID)
	assert.Nil(t, err)
	assert.NotNil(t, executorFromDB)
	assert.Equal(t, "", executorFromDB.NodeID)
}

func TestMultipleExecutorsOnSameNode(t *testing.T) {
	db, err := PrepareTests()
	assert.Nil(t, err)
	defer db.Close()
	...
	// Create multiple executors on the same node
	executor1 := utils.CreateTestExecutor(colony.Name)
	executor1.NodeID = node.ID
	err = db.AddExecutor(executor1)
	assert.Nil(t, err)

	executor2 := utils.CreateTestExecutor(colony.Name)
	executor2.NodeID = node.ID
	err = db.AddExecutor(executor2)
	assert.Nil(t, err)

	executor3 := utils.CreateTestExecutor(colony.Name)
	executor3.NodeID = node.ID
	err = db.AddExecutor(executor3)
	assert.Nil(t, err)

	// Verify all executors have the same NodeID
	executors, err := db.GetExecutorsByColonyName(colony.Name)
	assert.Nil(t, err)
	assert.Len(t, executors, 3)

	for _, exec := range executors {
		assert.Equal(t, node.ID, exec.NodeID)
	}
}
