...
func (db *PQDatabase) createProcessesIndex9() error {
	sqlStatement := `CREATE INDEX ` + db.dbPrefix + `PROCESSES_INDEX9 ON ` + db.dbPrefix + `PROCESSES (STATE, TARGET_EXECUTOR_NAMES, EXECUTOR_TYPE, IS_ASSIGNED, WAIT_FOR_PARENTS, TARGET_COLONY_NAME, PRIORITYTIME)`
	_, err := db.postgresql.Exec(sqlStatement)
	if err != nil {
		return err
	}
