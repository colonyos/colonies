...
func (db *PQDatabase) Initialize() error {
	...
	sqlStatement = `CREATE TABLE ` + db.dbPrefix + `RUNTIMES (RUNTIME_ID TEXT PRIMARY KEY NOT NULL, RUNTIME_TYPE TEXT NOT NULL, NAME TEXT NOT NULL, COLONY_ID TEXT NOT NULL, CPU TEXT, CORES INTEGER, MEM INTEGER, GPU TEXT NOT NULL, GPUS INTEGER, STATE INTEGER)`
	_, err = db.postgresql.Exec(sqlStatement)
	if err != nil {
		return err
	}
