...
func (db *PQDatabase) Initialize() error {
	...
	sqlStatement = `CREATE TABLE ` + db.dbPrefix + `PROCESSGRAPHS (PROCESSGRAPH_ID TEXT PRIMARY KEY NOT NULL, TARGET_COLONY_ID TEXT NOT NULL, ROOT TEXT NOT NULL, STATE INTEGER, SUBMISSION_TIME TIMESTAMPTZ, END_TIME TIMESTAMPTZ, RUNTIME_GROUP TEXT)`
	_, err = db.postgresql.Exec(sqlStatement)
	if err != nil {
		return err
	}
