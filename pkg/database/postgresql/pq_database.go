...
func (db *PQDatabase) Initialize() error {
	...
	sqlStatement = `CREATE TABLE ` + db.dbPrefix + `RUNTIMES (RUNTIME_ID TEXT PRIMARY KEY NOT NULL, RUNTIME_TYPE TEXT NOT NULL, NAME TEXT NOT NULL UNIQUE, COLONY_ID TEXT NOT NULL, CPU TEXT, CORES INTEGER, MEM INTEGER, GPU TEXT NOT NULL, GPUS INTEGER, STATE INTEGER, COMMISSIONTIME TIMESTAMPTZ, LASTHEARDFROM TIMESTAMPTZ, LONG DOUBLE PRECISION, LAT DOUBLE PRECISION)`
	_, err = db.postgresql.Exec(sqlStatement)
	if err != nil {
		return err
	}
	...
	sqlStatement = `CREATE TABLE ` + db.dbPrefix + `CRONS (CRON_ID TEXT PRIMARY KEY NOT NULL, COLONY_ID TEXT NOT NULL, NAME TEXT NOT NULL UNIQUE, CRON_EXPR TEXT NOT NULL, INTERVALL INT, RANDOM BOOLEAN, NEXT_RUN TIMESTAMPTZ, LAST_RUN TIMESTAMPTZ, WORKFLOW_SPEC TEXT NOT NULL, PREV_PROCESSGRAPH_ID TEXT NOT NULL, WAIT_FOR_PREV_PROCESSGRAPH BOOLEAN)`
	_, err = db.postgresql.Exec(sqlStatement)
	if err != nil {
		return err
	}
