...
func TestAddGetFile(t *testing.T) {
	...
	fileFromDB, err := db.GetFileByID(file.ID)
	assert.Nil(t, err)
...
func TestGetFileNamesByPrefix(t *testing.T) {
	...
	filesnames, err := db.GetFileNamesByPrefix("/testdir")
	assert.Nil(t, err)
	assert.Len(t, filesnames, 2)

	filesnames, err = db.GetFileNamesByPrefix("/testdir2")
	assert.Nil(t, err)
...
func TestDeleteFileByID(t *testing.T) {
	...
	file1FromDB, err := db.GetFileByID(file2.ID)
	assert.Nil(t, err)
	assert.NotNil(t, file1FromDB)

	err = db.DeleteFileByID(file2.ID)
	assert.Nil(t, err)
...
func TestDeleteFileByName(t *testing.T) {
	...
	files, err := db.GetFileByName(file4.Prefix, file4.Name)
	assert.Nil(t, err)
	assert.Len(t, files, 3)

	err = db.DeleteFileByID(file4.ID)
	assert.Nil(t, err)
	...
	err = db.DeleteFileByName(file4.Prefix, file4.Name)
	assert.Nil(t, err)
	...
	fileFromDB, err := db.GetFileByID(file4.ID)
	assert.Nil(t, err)
	assert.Nil(t, fileFromDB)

	fileFromDB, err = db.GetFileByID(file1.ID)
	assert.Nil(t, err)
